...
### state.md 初始化（Feature 级）

创建 `docs/backlog/{epic-id}/{feature-id}/state.md`：
...
### US 目录与 US 级 state.md 初始化

设计完成后，为每个 US 创建子目录和初始状态文件：
...
2. `state.md`（feature 级）已更新为 `current: Designed`
3. 每个 US 的子目录和 `state.md`（US 级）已创建
4. 用户明确 approve（设计方案审批通过）
5. 需要 Reviewer 预审的已得到回复
6. 涉及架构修改的已通过架构审批 Gate
...
| `state.md`（feature 级） | 是 | 初始化（`current: Draft`）；设计完成后追加 `Designed` |
| `us-*/state.md`（US 级） | 是 | 为每个 US 创建初始状态（`current: Designed`） |
| `.last-action-summary.md` | 是 | 供主 Agent 快速读取 |

### 完成信号

- **成功**：`.last-action-summary.md` 中标记 `suggested_state: "Designed"`，`needs_human_gate: true`；Orchestrator 在用户 approve 后统一更新 `state.current`
