...
depends_on: [test-design-rubric, test-execution, e2e-playwright]
...
- **E2E**：Playwright 规范见 `e2e-playwright` Skill
...
执行流程、状态定义、通知模板、test-report.md 编写规范、state.md 更新规则详见 `test-execution` Skill。
...
5. **state.md `code_paths` 回填**（按需）→ 合并到 main 的 PR diff 作事实源
